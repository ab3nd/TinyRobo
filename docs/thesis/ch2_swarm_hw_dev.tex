...
The TinyRobo platform development is not yet at a point where it could be offered as a finished product for hobbyists to purchase, but it is hoped that the lessons learned from its development will make the development of future cheap swarms easier and faster. 
...
Quantifying the relationship between the parameters of fiducial systems, such as camera quality, tag size, and camera calibration, with the output quality of the fiducial systems would likely be a help to other researchers as well. 
...
There is still at least a little work that could be done to improve the shape of the 3D base. 
