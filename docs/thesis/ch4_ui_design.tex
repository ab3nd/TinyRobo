...
\begin{figure}
	...
	\caption{UI gestures, selection gestures, and position gestures.}
...
Tap, lasso, box select, doubletap, drag, hold, and ``other'' cover a total of 91.9\% of the selection gestures.
...
This number of pixels was selected because it is the approximate width of a human finger on the screen used in this work. 
...
The selection of parameters like this for an interface intended for real-world, rather than experimental use, should be guided by understanding of human factors and UI design principles, such as Fitts' Law \citep{fitts1954information}.
...
Using a multi-finger gesture has the advantage of not adding UI elements, but is not easily discoverable by a user examining the UI, and so would require some form of training. 
...
As seen in Section \ref{section:Selection_Gestures} the use of selection gestures was low in the 1000 robot case, and much higher in the 10 robot case, implying that with a large robot group, it might make sense to default to using all robots if none are selected. 
...
The gestures used by the particpants in the experiment could be divided between selection and position gestures, within which there was a great deal of agreement, and more complex actions, such as patrol, formations, picking up or moving objects, removing robots, and selection by color. 
...
If the area the robots are displayed on and interacted with in does fit on the screen, it can be shrunk slightly, and the buttons can be placed so they do not cover the interaction area. 
...
In order to avoid having the system act prematurely, the system treats a double tap on open space as an ``end of command" signifier, as in \cite{micire2010multi}.
...
In addition to double taps, there are limited instances where it is safe for the system to assume that the user's command is complete. 
...
Similar heuristic rewriting of the stack could be extended to remove erroneous waypoints resulting from triple-tapping instead of double-tapping to end gestures, but adding these sorts of heuristics poses something of a threat. 
...
Expecting a sentence-like form for commands does not take as much advantage as it of simultaneous actions possibly could. 
...
However, two-handed gestures accounted for only 2.52\% of the gestures used.
