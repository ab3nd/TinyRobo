...
\todo{how many?} users had used double-tap for other purposes, but its relative scarcity made it a good candidate for reservation for this purpose. 
...
\todo{Do this and report back, but it's like 1AM now, so do it tomorrow}
...
%
%\end{tabular}
%\caption{Participant gesture sets by participant and task. Those marked with a -1 do not apply to that participant, such as the dispersion task in the single robot case. Those marked with a 0 contain gestures the gesture recognizer is not expected to recognize, while those marked with a 1 contain only recognizable gestures. }\label{tab:recognizable gestures}
%\end{table}
