...
Each TinyRobo has an April Tag \change{Cite this} on top of it, so that the central computer can localize them within the arena. 
...
The apriltags\_ros \change{Cite this} node publishes the detected locations of the tags in meters, but the computer vision detection of blue objects in the arena camera image operates in pixels. 
...
\begin{figure}[h]
	...
	vsc -> sub2 [label=<sensor\string_msgs/LaserScan>];
	vsc -> sub3 [label=<sensor\string_msgs/LaserScan>];
...
The individual TinyRobos do not have powerful processors. 
The majority of the processing is performed on a host computer running the ROS software framework. 
Each robot's processor is only concerned with receiving commands via WiFi and controlling the motors of the robot. 
