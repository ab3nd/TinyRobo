...
Each robot was run 5 times, starting from the same location each time. The robot was power-cycled between runs. Each run was recorded using rosbag, and the data from the recordings were used to generate visualizations of the robot's commanded velocity and measured velocity from Apriltag tracking.
...
ROS provides tools for removing the distortion inherent in the use of a wide-angle lens, but at a cost of decreased effective resolution at the edges of the image. 
...
This solution ends up requiring more computational power as the image size increase, in order to keep the tag detection framerate high enough to be useful for realtime control. 
...
Acceleration during the movement phases was managed by increasing the commanded velocity of the robot until the April tag tracking detected that the robot had begun moving. 
...
The big wheel robot displayed an unfortunately small range of commanded velocities between those which caused it to begin moving, and those which caused it to flip over on its side.
...
\begin{figure}
	\centering
	\includegraphics[width=0.8\textwidth]{robot_1.png}
	\caption{Motion of big wheel robot}
\end{figure}

The Hexbug-branded 6-wheeled bug displayed an asymmetry in its motor drive speeds. 
...
In one run, one side didn't begin moving at all, causing the robot to rotate rapidly around that side. 
...
\begin{figure}
	\centering
	\includegraphics[width=0.8\textwidth]{robot_6.png}
	\caption{Motion of 6-wheel bug}
\end{figure}

The green tank, carrying the 0 Apriltag, experienced problems with one side of its drive train in three of its five runs.
...
\begin{figure}
	\centering
	\includegraphics[width=0.8\textwidth]{robot_0.png}
	\caption{Motion of robot 0}
\end{figure}

There are two blue tanks, carrying Apriltags numbered 8 and 18. 
Tank number 8 moved slightly on four of its 5 runs, and did not move on one of them. 
At no point did it move the full 0.25m. 
Tank number 18 moved much more consistently, but displayed an arc to the right in four of its 5 runs. 
...
\begin{figure}
	\centering
	\includegraphics[width=0.45\textwidth]{robot_8.png}
	\includegraphics[width=0.45\textwidth]{robot_18.png}
	\caption{Motion of robots 8 (left) and 18 (right)}
\end{figure}

The single-motor Hexbug-branded blue bug moved consistently, but with a heavy bias towards turning to the left. 
In every run, it started and stopped, but the curvature of the path to the left caused it to run into one side of the arena or the other. 
In one run, it fell over while moving, due to the somewhat ``bouncy'' nature of the toy's gait.

\begin{figure}
	\centering
	\includegraphics[width=0.8\textwidth]{robot_5.png}
	\caption{Motion of bug robot}
\end{figure}

The Ackerman-steering toy car also displayed very consistent mobility. In four out of its five runs, it crossed the arena without incident. 
...
\begin{figure}
	\centering
	\includegraphics[width=0.8\textwidth]{robot_17.png}
...
The tendency of the big wheel robot to flip over could be mitigated by very gradually increasing its speed, at the cost of reducing its control responsiveness. 
Increasing the frequency of the feedback loop that checks its speed, perhaps by parallelizing the April tag library, would allow it to be operated in the narrow range of stable speeds. 
