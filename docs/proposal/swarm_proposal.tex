...
Methods for command and control that are based on issuing individual orders to individual actors do not scale to large numbers of actors \unsure{Cite this}. 
...
Bergbreiter's COTSBots use TinyOS, a modular and event-driven framework \improvement{cite TinyOS paper} for developing node software. 
...
This time is called "neglect time"\unsure{cite paper that defines this}.
...
\section{Timeline}
 ...
 - Collect gesture set. Run IRB paperwork, Test experiment with lab users. GEt a lot of gift cards. Print up flyers, announce to all my friends. \improvement{Formal description of the experiment. Recruiting protocol. Preliminary tests.}
 
 - Finish swarm control hardware. Test motor driver hardware, fix known bugs, respin board. Wifi connection to motors. 
 \improvement{Describe what has already been done. Describe what needs to be done. Measure motor currents to ensure driver is sufficient.}
 
 - Write swarm control software framework. Pretty much a straight up coding task. Simplest thing that could possibly work is probably individual robots driving in a circle, or otherwise all running the same code. Then add virtual sensing. Then virtual networking (extension of sensing, network is a sense). Robot localization is also a sense (sense of place). Then virtualized failures. \improvement{Create some graphics of the proposed system.}
