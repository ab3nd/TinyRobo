...
One potential method for control of a swarm to perform a task is allowing a central computer to dictate to individual robots how to move.
...
Emergent behaviors are those behaviors of a system that are irreducible to the actions of a single individual, but arise from the interactions of multiple individuals \cite{tsuda2004robust}.
...
While first responders are frequently trained in many specialized skills to perform their jobs, adding coding of emergent behavior algorithms to that training is absurd. 
...
As a result, it is desirable to automate the construction of control software for a swarm so that it can adapt to a situation, without requiring significant development time during the situation in which the software is expected to be used. 

Assumptions: fire-and-forget deployment of software to nodes, possibly using a distributed distribution method similar to bittorrent. Nodes are updated as possible. Network connectivity is range limited. Network may not be completely connected. Robots can fail at any time. Robustness is achieved through redundancy. Sensing is local and range-limited. 
...
Instead, motion within a space is usually performed as part of accomplishing a task.
For example, searching an area requires moving over the area to be searched.
However, specialized tasks will likely require specialized interfaces to provide the information that is specific to that task. 
...
Once the ratio of the area the swarm is in to the area of the map displayed to the user becomes sufficiently large, displaying the swarm members at the same scale as the map will result in the representation of the swarm members being too small to interact with. 
...
Scaling the representation up makes the robot representations overlap unrealistically. 
...
Once the language is defined, sequences of gestures in that language must be translated into programs to execute on each member of the swarm. 
The program for each swarm member must be generated automatically, either by creation of a new program or by a synthetic approach from pre-determined behavior primitives.
When providing motivating examples for research in swarm robotics, the focus is on the use of the robots, rather than the programming approach used to control them. 
End users are not concerned about programs, but about accomplishing tasks. 
...
The hardware resulting from this work will be designed so that it can be duplicated by a researcher using common tools, and possessed of no more than hobby-level familiarity with hardware.
