...
Control of swarms to perform a task is accomplished by a central computer dictating to well-localized robots how to move. 
These systems are usable by non-software-developers, but the use of centralized control systems creates a single point of failure. 
Also, the assumption that all of the robots are well-localized and in constant contact with the central controller frequently does not obtain in the real world. 

Emergent control of swarms is guided by ad-hoc, iterative development models that are somewhat suited to software developers, but not suited to use by non-programming end users. 
Emergent control systems do not have the single point of failure that centralized models have. 
...
Altering how the user interface displays the location of the robots in the swarm will affect the transition point. More specifically, a display which obscures individual robots and displays a cloud or swarm boundary will cause the user to treat the swarm as a whole rather than individuals. 
...
More specialized tasks, such as searching for a target item within a specific area, will likely require specialized interfaces to provide the information that is specific to that task. 
...
Even if there is not an intuitive command for a task, it is possible to compare selected command gestures for usability. 

In order to determine if the intuitive gestures change with the size of the swarm, tests can be conducted with varying swarm sizes performing the same tasks. 
The swarm should be large, with the lower bound on its size being significantly larger than the number of fingers a user could gesture with. 
...
The size of the swarm where changes in the user interaction method occur will indicate the transition point between interaction with individual robots and interaction with the swarm as a whole. 
...
In order to locate the robots within the experiment area, an overhead camera will be used to detect QR codes on each robot. 
...
Rather than controlling each robot, the central computer maintains a separate process for each robot in the swarm, which will be allowed to drive only the robot that it is linked to. 
...
The code operating in visualization on the robot may be neither aware of its own position in the world, or the location of other robots, if the experiment calls for such a lack of information. 
...
The virtual sensors can also be configured to emulate error conditions such as noisy sensors, missing sensors, degraded localization, and so forth.
...
If the robots are required to communicate directly with each other, the communication will pass through a virtual network, similar to the virtual sensors. 
...
Multitouch gesture set for command of multiple robots.
Determination of whether gestures or neglect of individual robots change with scale of swarm.
Compilation of gestures into command programs for robots.

\subsubsection{\emph{En Passant} Contributions}

In chess, \emph{en passant} is a special pawn capture that can be used to capture a pawn that advances two ranks from its start position, passing an opponent pawn. 
...
The intent of this device is to have an inexpensive platform available for swarm research without any swarm robotics researchers having to start and maintain a side business supporting and selling robots (much as the author might appreciate it).
...
The plans and information required to reproduce Jasmine units are available for free at Swarmrobot.org, but the chassis of Jasmine is a custom mechanical assembly, rather than a commercially available product. 
...
At the time of this writing, the particular car used is moderately expensive for a toy car, although quite cheap for a research robot, costing a little over \$100USD per unit. 
...
For example, obstacles can be created by drawing lines on the floor of the robot enclosure. 
Different colors could represent different types of obstacles, or qualities of the obstacles that are relevant to the software under test. 
It should be stressed that while the system as a whole can localize the robots, both relative to each other and by absolute position within the arena, this information may be withheld from the individual robots. 
The code operating in visualization on the robot may be neither aware of its own position in the world, or the location of other robots, if the experiment calls for such a lack of information. 
...
However, heterogeneity can also be leveraged to increase the robustness of a swarm and reduce its cost\change{cite something to demonstrate this}. 
...
%EID is hierarchical in the same way that a lot of the programming approaches to the development of swarms are heirarchical. Primitive behaviors at the bottom, embedded in larger contexts at the middle level, and connected to overall task at the top.
...
At the base level is the behavior explicit in the program and the actions generatedAnother by it in the environment. 
