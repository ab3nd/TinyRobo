...
Methods for command and control that are based on issuing individual orders to individual robots do not scale to large numbers of robots \unsure{Cite this}. 
...
Any software that purports to control a swarm under these conditions cannot rely on perfect connectivity for its operation \unsure{cite this}. 
...
In a situation where many or all of the robots attempting to complete a task are destroyed or disabled, it would be unreasonable to expect the task to be completed, but ideally, the bounds on how many robots are required to complete the task should be knowable\improvement{Look for prior research on this}. 
...
This problem will arise at smaller scales if the swarm robots are themselves quite small, and since making swarm robots smaller is an active area of research \unsure{Find an overview paper, cite that}, this problem will only get worse. 
...
The robots used in most swarm work are of a sufficently small size that many of them can fit in a room. In addition to budgetary constraints, interaction with an environment built for humans places and upper bound the scale of the individual swarm members.  \unsure{find a robot size taxonomy and cite that}
...
Over the course of the project from 2004-2008, the hardware was developed and used in research, but was not converted to a commercial product. \improvement{Cite that sandia (?) maglev manipulation platform}
...
COTSBots use TinyOS, a modular and event-driven framework \improvement{cite TinyOS paper} for developing node software. 
...
The MarXbot swarm platform \improvement{cite that heterogenous team robot system that was based on these} is capable of operating in unstructured human environments. MarXBots can also use their grippers to link themselves together and perform operations \improvement{Such as?} that an individual robot could not perform \cite{bonani2010marxbot}. However, it has a strong computer and a rich set of sensors and effectors, but as a result it is quite expensive. 
...
\improvement{Find someone who did amigabot swarm or roomba swarm, and cite that work here. I know it exists, I saw videos.}
...
For the purposes of this research, the user interface is a multitouch surface \unsure{cite some early work on multitouch interfaces} that displays representations of the area the swarm is in and of the individual swarm robots. 
...
This time that the robot operates without interaction from the user is called ``neglect time"\unsure{cite paper that defines this}.
...
Worse, the user may make errors because of an incorrect understanding of the system when they begin operations after a long neglect time\unsure{cite this, surely someone has measured error/neglect time}. 
...
If the user is overloaded, their attention will become subject to triage, and they will begin to miss elements of the task \unsure{cite}. 
...
However, in situations with even moderate numbers of robots, even relatively high levels of automation may overwhelm the user \unsure{cite the papers on fanout in UAVs}. 
...
Instead, the amount of time the user spends interacting with the robots begins to outweigh the neglect time, and so the robots spend increasing amounts of time waiting for interactions\change{cite whatever defines neglect time}. 
...
Without direct feedback, the user has to wait to see if the robots are behaving as expected, which adds a delay to the user's interaction with the system and introduces uncertainty.\improvement{cite}
...
The experiment will be run before finals week. 
For the experiment, there are 5 cases: 1 robot, 10 robots, 100 robots, 1000 robots, cloud (unknown robot count). 
5-10 users per case would be good, so I hope to recruit 25-50 users. 
...
Hardware - The swarm control board currently has problems related to the programming interface. A second version of the swarm board is needed, to correct these problems. 
Once that is done, the new version can be used to test the motor drivers in all the swarm robot chassis' currently available. 
The robot enclosure needs one or more cameras mounted over it.

Software - Testing the motor drivers will require a first version of the motor driver firmware. 
Connecting to all the robots in order to control them should require minimal effort on the part of the user. 
Ideally, all of the robots will connect to a single access point, and the control computer will be networked to that AP. 
Discovery of the robots will be automated. 
...
Writing - Proposal will be revised. 

\subsection{Summer 2016}

Gestures - Assuming sufficient data for analysis of the gesture set has been collected, it will be analyzed and compared to existing work on multitouch gestures for command and control. 
...
Otherwise, further data will be collected over the summer. 

Hardware - The bugs in version one of the swarm control boards will be fixed. 
Version two will be tested. 
If it works, the boards can be mass-assembled. 
Otherwise, a third version should be sufficient to settle any remaining design problems.
...
This level of functionality, enabling individual swarm members to be driven from the keyboard, will be completed in the summer. 
The automatic swarm configuration will be extended to bluetooth, so that the Epucks can be added to the swarm as well. 
Creation of configuration files that load ROS nodes to speak to each individual robot will be automated. 
...
Gestures - If the amount of data collected from the first run of the gesture tests was insufficient to get useful results, collect a second set of data. 
Compare to first data, and determine if combined data set has sufficent data to get useful results. 
