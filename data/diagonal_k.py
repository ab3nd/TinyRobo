...
def register_lists(list1, list2):
	...
	events_1 = sorted(list1, key=lambda event: event['time'])
	events_2 = sorted(list2, key=lambda event: event['time'])
	...
	for idx, event in enumerate(best_match):
		if event is None:
...
for task1, task2 in zip (tasks_1, tasks_2):
	...
	list1, list2 = register_lists(events_1, events_2)
...
#First the top row
print "," + ",".join(["{1} ({0})".format(coder_1, name) for name in names])
#Then each row with its values
for firstName in names:
	line = firstName + " ({})".format(coder_2) + ","
	values = []
	for secondName in names:
